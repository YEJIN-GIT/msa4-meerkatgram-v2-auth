...
import com.msa4meerkatgramv2auth.global.config.SubServiceURIConfig;
...
public class OAuth2FailureHandler extends SimpleUrlAuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull AuthenticationException exception) throws IOException, ServletException {
        ...
        logger.debug("OAuth2 Error: " + errorCode, exception);  // 개발환경
        if(errorCode.equals(CustomResponseCode.OAUTH2_ERROR.getCode())) {
            logger.error("OAuth2 Error: " + errorCode, exception);  // 운영환경
        }

        // 환경설정 FRONTENT_CALLBACK_URI=http://localhost:5173/oauth2/callback
        // 오류발생시 프론트로 콜백 주소 http://localhost:5173/oauth2/callback?code=E30&message=OAUTH2_ERROR
        // 쿼리 파리메터 URL 인코딩
